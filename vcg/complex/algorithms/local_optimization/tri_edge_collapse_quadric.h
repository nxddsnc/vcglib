...
>>>>>>> 7e657e07fc7873bdd2599cefec538c3387a75a17
    ...
    public:
      ...
      static void Init(TriMeshType &m, HeapType &h_ret, BaseParameterClass *_pp, std::unordered_map<VertexType*, VertexType*>& vertexCachePair)
      {
        QParameter *pp = (QParameter *)_pp;
              ...
              else
              {
                if (vi != vi1 && !(*vi).IsD() && (*vi).IsRW())
                {
                  ...
                  if (test < pp->CollapseThr)
                  {
                    ...
                    vertexCachePair.insert(std::make_pair(&*vi, &*vi1));
                  }
                }
              }
            }
...
>>>>>>> 7e657e07fc7873bdd2599cefec538c3387a75a17
      ...
      inline  void UpdateHeap(std::unordered_map<MyVertexType*, MyVertexType*>& vertexPairCache, HeapType & h_ret, BaseParameterClass *_pp)
      {
        ...
        for (VFIterator vfi(v[1]); !vfi.End(); ++vfi) {
          if (!(vfi.V1()->IsV()) && vfi.V1()->IsRW())
          {
            ...
            if (vertexPairCache.count(vfi.V1()))
            {
              VertexType* vertex = vertexPairCache.at(vfi.V1());
              float distance = Distance(vfi.V1()->P(), vertex->P());
          ...
          if (!(vfi.V2()->IsV()) && vfi.V2()->IsRW())
          {
            ...
            //if (vertexPairCache.count(vfi.V2()))
            //{
            //    VertexType* vertex = vertexPairCache.at(vfi.V2());
            //    float distance = Distance(vfi.V2()->P(), vertex->P());
            //    if (distance < pp->CollapseThr)
            //    {
